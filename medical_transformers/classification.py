...
def parse_arguments():
    ...
    parser.add_argument(
        "--find_lr", action="store_true", default=False, help="Flag for lr finder"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
...
def parse_arguments_jupyter(args):
    parser = argparse.ArgumentParser(
        description="The main takes as \
                             argument the parameters dictionary from a json file"
    )
...
def get_jupyter_items(train=True):
    args = parse_arguments_jupyter(args=[])
    ...
    return trainer.get_train() if train else trainer.get_test()
