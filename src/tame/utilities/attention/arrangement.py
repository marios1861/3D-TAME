...
def tv_loss(masks, var_coeff):
    var_loss = torch.tensor(0.0).to(masks.device)
    for i in range(masks.ndim - 2):
        last_elem = masks.size(i + 2) - 1
        var_loss += torch.abs(
            torch.index_select(
                masks, i + 2, torch.arange(1, last_elem).to(masks.device)
            )
            - torch.index_select(
                masks, i + 2, torch.arange(0, last_elem - 1).to(masks.device)
            )
        ).mean()
    var_loss /= masks.ndim - 2
    mean_dist = torch.mean(torch.square(masks))
    return var_coeff * var_loss + mean_dist, var_loss, mean_dist


class Arrangement(nn.Module):
    r"""Defines train_policy and get_loss components of Generic"""
    arrangement_types = Union[
        Literal[
            "trada",
            "adapt_all",
            "adapt",
    ...
    def __init__(self, version: arrangement_types, body: nn.Module, output_name: str):
        super(Arrangement, self).__init__()
        arrangements = {
            "trada": (self.new_train_policy, self.trada_loss),
            "manual": (self.new_train_policy, self.manual_loss),
            "adapt_all": (self.new_train_policy, self.ada_all_loss),
            "adapt": (self.new_train_policy, self.ada_loss),
            "betanew": (self.new_train_policy, self.new_loss),
        ...
        elif version == "adapt_all" or version == "trada":
            self.new_coeff = 0.1
            self.softadapt_object = LossWeightedSoftAdapt(beta=0.1)
            self.steps_to_make_updates = 10
            self.count = 0
        ...
        elif "manual" in version:
            print(version)
            version, weights = version.split("_")
            weights = weights.split(",")
            self.ce_coeff = float(weights[0])
            self.area_loss_coeff = float(weights[1])
            self.smoothness_loss_coeff = float(weights[2])
        else:
    ...
    def trada_loss(
        ...
        labels = labels.long()
        variation_loss = Arrangement.var_loss(masks)
        area_loss = self.area_loss(masks)
        cross_entropy = self.loss_cross_entropy(logits, labels)

        self.values_of_component_1.append(cross_entropy)
        self.values_of_component_2.append(variation_loss)
        self.values_of_component_3.append(area_loss)
        ...
        loss = (
            self.adapt_weights[0] * cross_entropy
            + self.adapt_weights[1] * variation_loss
            + self.adapt_weights[2] * area_loss
        )
    ...
    def manual_loss(
        ...
        loss = (
            ...
            + self.area_loss_coeff * area_loss
            + self.smoothness_loss_coeff * variation_loss
        )
    ...
    @staticmethod
    def var_loss(masks, power=2, border_penalty=0.3):
        if masks.dim() == 4:
            B, C, _, _ = masks.size()
            x_loss = torch.mean(
                (torch.abs(masks[:, :, 1:, :] - masks[:, :, :-1, :])) ** power
            ...
            if border_penalty > 0:
                border = float(border_penalty) * torch.mean(
                    masks[:, :, -1, :] ** power
                    + masks[:, :, 0, :] ** power
                    + masks[:, :, :, -1] ** power
                    + masks[:, :, :, 0] ** power
            ...
            if border_penalty > 0:
                ...
                border = 0.0
            return x_loss + y_loss + z_loss + border
