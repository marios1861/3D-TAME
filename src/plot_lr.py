...
def plot(json_file, name: str, window=None):
...
def main():
    ...
    lr_load_dir = ROOT_DIR / "LR"
    file_list = []
    for entry in lr_load_dir.iterdir():
        if entry.is_file() and "json" in entry.suffix:
            file_list.append((entry.stem, entry))

    lr_save_dir = ROOT_DIR / "LR"
    lr_save_dir.mkdir(parents=True, exist_ok=True)
    compare = input("Do you want to compare multiple graphs? [y/n]\n")
    window = input(
        "Do you want to also show the moving average? If so, input window size:\n"
    )
    ...
    if not (compare == "y" and save == "y"):
        ...
        if compare == "n":
            file = input(
                "Select a data file from the list:\n    "
                + "\n    ".join(
                    [f"{i + 1}) {name}" for i, (name, _) in enumerate(file_list)]
                )
                + "\n"
            )
            try:
                name, path = file_list[int(file) - 1]
                fig = plot(path, name, window)
                if save == "y":
                    fig.savefig(
                        os.path.join(lr_save_dir, name.replace(".json", ".png"))
                    )
            ...
            file_idxs = input(
                "Input a list of numbers delimited by , for each file to compare from the list:\n    "
                + "\n    ".join(
                    [f"{i + 1}) {name}" for i, (name, _) in enumerate(file_list)]
                )
                + "\n"
            ...
            try:
                ...
                (names, paths) = (
                    [file_list[int(j) - 1][i] for j in file_idxs] for i in range(0, 2)
                )
