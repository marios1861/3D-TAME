...
def get_optim(cfg: Dict[str, Any], model: Generic) -> optim.Optimizer:
    ...
    if cfg['optimizer'] == 'Adam':
        ...
        optimizer = optim.AdamW(g[2], lr=0.1, betas=(cfg['momentum'], 0.999),
                                weight_decay=0.0)
    elif cfg['optimizer'] == 'RMSProp':
        optimizer = optim.RMSprop(g[2], lr=0.1, momentum=cfg['momentum'])
    elif cfg['optimizer'] == 'SGD':
        optimizer = optim.SGD(g[2], lr=0.1, momentum=cfg['momentum'],
                              nesterov=True)
    else:
        raise NotImplementedError(
            f'Optimizer {cfg["optimizer"]} not implemented.')
...
def get_schedule(cfg: Dict[str, Any],
                 optimizer: optim.Optimizer,
                 steps_per_epoch: int,
                 currect_epoch: int) -> lr._LRScheduler:
    return lr.OneCycleLR(optimizer, cfg['lr'], epochs=cfg['epochs'],
                         steps_per_epoch=steps_per_epoch,
                         # this denotes the last iteration, if we are just starting out it should be its default
                         # value, -1
                         last_epoch=(currect_epoch * steps_per_epoch)
                         if currect_epoch != 0 else -1)
